...
public class CacheUtil {
	 ...
	 * @param key
	 */
	public static void initSynch(String key) {
		CacheSynch cs=new CacheSynch();
		cs.setOid(key);
	...
	public static String executeSynch(String key,int n) throws Exception {
		...
		try {
			int num=new SqlBuffer(6).append("update z_cache_synch set synch_status=1 where oid=? and synch_status=0", key).executeUpdate();
			if(num<1) {//未更新成功
